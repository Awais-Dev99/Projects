...
/**
 * Sub-component to handle search params and form logic.
 * This must be wrapped in Suspense to avoid build errors.
 */
function LoginForm() {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  ...
  return (
    <div className="w-full max-w-md bg-white p-6 md:p-10 rounded-[2.5rem] shadow-2xl shadow-blue-900/5 border border-gray-100 transition-all">
      ...
      {/* Success Message from Search Params */}
      {message === "pending" && (
        <div className="bg-amber-50 text-amber-700 p-4 rounded-2xl mb-6 text-sm border border-amber-100 flex items-start gap-3">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 shrink-0" viewBox="0 0 20 20" fill="currentColor">
            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
...
export default function LoginPage() {
  return (
    <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-slate-50 px-4">
      ...
      {/* 
        Wrapped in Suspense to prevent "missing-suspense-with-csr-bailout" 
        error during Vercel build. 
      */}
      <Suspense fallback={
        <div className="w-full max-w-md bg-white p-10 rounded-[2.5rem] shadow-xl flex items-center justify-center">
